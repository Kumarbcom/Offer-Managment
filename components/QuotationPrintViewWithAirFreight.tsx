                    ...
                    <table className="w-full text-left text-[10px] border-collapse">
                        <thead className="bg-slate-900 text-white uppercase tracking-wider text-[9px]">
                            <tr className="divide-x divide-slate-850">
                                ...
                                <th className="p-2 font-bold align-middle w-1/4" rowSpan={2}>Description</th>
                                <th className="p-2 font-bold text-center w-8 align-middle" rowSpan={2}>MOQ</th>
                                <th className="p-2 font-bold text-center w-8 align-middle" rowSpan={2}>REQ</th>
                                <th className="p-2 font-bold text-center w-8 align-middle" rowSpan={2}>UOM</th>
                                <th className="p-2 font-bold text-right w-14 align-middle" rowSpan={2}>Unit Price (₹)</th>
                                <th className="p-2 font-bold text-right w-16 align-middle" rowSpan={2}>Amount (₹)</th>
                                <th className="p-2 font-bold text-center w-14 align-middle" rowSpan={2}>Stock Status</th>
                                <th className="p-2 font-bold text-center" colSpan={3}>Air Freight Details</th>
                                <th className="p-2 font-bold text-right w-18 align-middle" rowSpan={2}>Total (₹)</th>
                            </tr>
                            <tr className="divide-x divide-slate-850 border-t border-slate-800">
                                <th className="p-1.5 font-bold text-right w-12">Per Unit (₹)</th>
                                <th className="p-1.5 font-bold text-right w-14">Amount (₹)</th>
                                <th className="p-1.5 font-bold text-center w-14">Lead Time</th>
                            </tr>
                        </thead>
                            ...
                            {(quotation.details || []).map((item, index) => {
                                ...
                                return (
                                    <tr key={index} className="divide-x divide-slate-200 hover:bg-slate-50/50 transition-colors odd:bg-slate-50/20">
                                        ...
                                        <td className="p-2 text-right text-slate-800">{amount.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                                        ...
                                        <td className="p-2 text-right text-slate-600 font-semibold">{freightPerUnit > 0 ? freightPerUnit.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '-'}</td>
                                        <td className="p-2 text-right text-indigo-700 font-semibold bg-indigo-50/5">{freightAmount > 0 ? freightAmount.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '-'}</td>
                                        <td className="p-2 text-center text-slate-500">{(item.airFreight && item.airFreightDetails) ? item.airFreightDetails.airFreightLeadTime : '-'}</td>
                                        <td className="p-2 text-right font-extrabold text-slate-900">{totalWithFreight.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                                    </tr>
                                );
                            })}
                        </tbody>
                ...
                <section className="flex justify-end my-4">
                    <div className="w-96 space-y-1.5 border border-slate-200/80 p-3 rounded-lg bg-slate-50/40 shadow-sm">
                        <div className="flex justify-between text-[10px] text-slate-600">
                            ...
                            <span className="font-bold">₹{totals.totalAmount.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                        </div>
                        <div className="flex justify-between text-[10px] text-slate-600">
                            <span className="font-semibold">Total Air Freight</span>
                            <span className="font-bold">₹{totals.totalFreight.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                        </div>
                        {quotation.gstAdded && (
                            <div className="flex justify-between text-[10px] text-slate-600">
