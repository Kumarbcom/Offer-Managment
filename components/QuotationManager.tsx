...
export const QuotationManager: React.FC<QuotationManagerProps> = ({ quotations, customers, salesPersons, setEditingQuotationId, setView, setQuotations, currentUser, quotationFilter, onBackToCustomers, globalSearch, onClearGlobalSearch }) => {
  ...
  const [emailModalQuotation, setEmailModalQuotation] = useState<Quotation | null>(null);
  ...
  const handleWhatsAppShare = (q: Quotation) => {
    const sp = salesPersons?.find(s => s.id === q.salesPersonId);
    if (!sp || !sp.mobile) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-indigo-50/30 font-sans">
      ...
      <div className="px-4 md:px-6 py-5 space-y-5">
              ...
              <div key={q.id} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                ...
                <div className="p-3">
                  ...
                  <div className="flex justify-end gap-3 border-t border-slate-100 pt-2 mt-2">
                    <button onClick={() => setEmailModalQuotation(q)} className="text-orange-600 text-xs font-medium hover:text-orange-700">Email</button>
                    <button onClick={() => handleWhatsAppShare(q)} className="text-emerald-600 text-xs font-medium hover:text-emerald-700">WhatsApp SP</button>
                      ...
                      <td className="px-3 py-1.5 text-right">
                        <div className="flex items-center justify-end gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
                          ...
                          <button
                            ...
                            className="w-7 h-7 rounded-lg flex items-center justify-center text-orange-600 hover:bg-orange-50 transition-colors"
                          >
                            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                            </svg>
                          </button>
                          {/* Removed redundant Notify SP button */}
                          {/* Edit */}
                          <button
                            onClick={() => handleEdit(q.id)}
        ...
        <EmailDraftModal 
          isOpen={!!emailModalQuotation}
          onClose={() => setEmailModalQuotation(null)}
          quotation={emailModalQuotation}
          salesPerson={emailModalQuotation ? (salesPersons?.find(s => s.id === emailModalQuotation.salesPersonId) || null) : null}
