                ...
                {filteredSOs.length > 0 ? filteredSOs.map((item, idx) => {
                    ...
                    return (
                        <tr key={item.id || idx} className="hover:bg-gray-50">
                            <td className="px-4 py-3 text-sm text-gray-900">{item.date ? new Date(item.date).toLocaleDateString() : 'N/A'}</td>
                            ...
                            <td className={`px-4 py-3 text-sm font-semibold ${isOverdue ? 'text-red-600' : 'text-green-600'}`}>
                                {item.dueOn ? new Date(item.dueOn).toLocaleDateString() : 'N/A'}
                            </td>
